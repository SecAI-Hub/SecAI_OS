...
- **Loopback/internal-only binding**: Appliance systemd units bind peer services to `127.0.0.1`. The Docker sandbox binds selected service ports inside the private Compose network so containers can talk to each other, while only the UI is published to the host by default.
